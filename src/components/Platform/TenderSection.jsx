  ...
  return (
    <section className="w-[95%] mx-auto py-20 grid lg:grid-cols-2 gap-10">
      <div className="bg-[#4A4A4A] p-12 rounded-[3.5rem] text-white relative overflow-hidden">
        ...
        <h3 className="text-3xl font-black mb-8  uppercase tracking-tighter"><span className="normal-case">e-Auction</span> <span className="text-[#4A4A4A]">Portal</span></h3>
        <div className="space-y-4">
          {tenders.map(t => (
            <div key={t.id} className="bg-white/5 p-6 rounded-3xl border border-white/10 flex justify-between items-center group hover:bg-[#D48282]/20 transition-all cursor-pointer">
              <div>
                <span className="text-[10px] font-black text-[#4A4A4A] uppercase tracking-widest">{t.id}</span>
                <p className="font-bold text-lg">{t.title}</p>
              </div>
              <div className="text-right">
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${t.status === 'Live' ? 'bg-green-500 animate-pulse' : 'bg-[#D48282]'}`}>{t.status}</span>
                <p className="text-[10px] text-gray-400 mt-2 flex items-center gap-1 justify-end"><Clock size={10}/> {t.end}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-[#FDF2F2] p-12 rounded-[3.5rem] border border-pink-100 flex flex-col justify-center">
        ...
        <h3 className="text-3xl font-black text-[#4A4A4A] mb-4">Tender & <span className="text-[#4A4A4A]">Contracts</span></h3>
        ...
        <button className="bg-[#D48282] text-white px-10 py-4 rounded-full font-black text-xs uppercase tracking-widest hover:bg-[#4A4A4A] transition-all self-start shadow-xl shadow-pink-100">
          View All Tenders
        </button>
      </div>
    </section>
